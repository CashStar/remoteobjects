...
a string that contains the response entity body.
        ...
        if redirections is not None and redirections > 0:
            config=dict(max_redirects=redirections)
            allow_redirects=True

        response = requests.request(method=method, url=uri, headers=headers,
            data=body, config=config, allow_redirects=allow_redirects)
